...
# Partie 4208 (Côté client) — Améliorations (faites)

## 1. Option « inclure les frais de retrait lors de l'envoi » (fait)
- Dans l'espace client (`/client/operations`), ajout d'une case à cocher « Inclure les frais de retrait dans le montant (vous recevez le montant saisi) » lors d'un retrait.
- Si cochée : le montant saisi est le net reçu, on débite `montant + frais`. Si décochée : comportement actuel (net = montant - frais).
- `Client::executer` lit `frais_inclus` et affiche un message de succès adapté (net reçu + frais).
- Fichiers : `app/Views/client/operations.php`, `app/Controllers/Client.php`.

## 2. Envoi multiple vers plusieurs numéros (montant divisé) (fait)
- Dans `/client/operations`, le bloc destinataire propose un textarea pour saisir plusieurs numéros (virgule, espace, saut de ligne) en plus du select contact.
- Le montant total est divisé équitablement (`montant_total / nb_numéros`, reliquat sur le premier).
- `Client::executerTransfert` : valide les préfixes, déduplique, vérifie le solde total (montants + frais + commissions), crée les destinataires absents, utilise une transaction DB.
- Une transaction par destinataire est enregistrée. Le solde de l'émetteur est débité une seule fois par destinataire dans la transaction.
- Fichiers : `app/Views/client/operations.php`, `app/Controllers/Client.php`.
